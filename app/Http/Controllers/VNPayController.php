    ...
    if ($secureHash === $vnp_SecureHash) {
        if ($request->vnp_ResponseCode == '00') {
            ...
            try {
                ...
                // Khai báo mảng tickets ở ngoài để lưu tất cả vé
                $allTickets = [];
                ...
                foreach ($eventGroups as $eventId => $group) {
                    ...
                    foreach ($group['items'] as $item) {
                        ...
                        for ($i = 1; $i <= $item['quantity']; $i++) {
                            ...
                            // Thêm vào mảng allTickets
                            $allTickets[] = [
                                'ticket_code' => $ticketCode,
                                'qr_code' => $qrCode,
                                'attendee_name' => $orderInfo['fullName'] ?? '',
                                'ticket_type' => $item['ticketTypeName'] ?? '',
                                'event_name' => $item['event_name'] ?? '', // Thêm tên sự kiện nếu có
                            ];
                        }
                    }
                ...
                // Tạo order với đầy đủ thông tin tickets
                $order = [
                    ...
                    'cart' => $cart,
                    'tickets' => $allTickets, // Gán tickets ở đây
                    'notes' => $orderInfo['notes'] ?? '',
                    'status' => 'paid',
                    'paymentMethod' => 'vnpay',
                ...
                try {
                    ...
                    Log::info('Email sent successfully to: ' . $order['customerInfo']['email']);
                } catch (\Exception $e) {
